...
class ViajeroPaqueteHistorial extends ObjectModel
{
    ...
    public $id_package_butaca_tipo_cupo;
    public $payment;
    public $reference;
    public $state;
    public $id_customer;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_package_butaca_tipo_cupo' => array('type' => self::TYPE_INT, 'required' => true),
            ...
            'impuesto' => array('type' => self::TYPE_FLOAT, 'required' => true),
            'fileIdRedEvt'=> array('type' => self::TYPE_INT, 'required' => false),
        ),
    );
...
//        var_dump($result);exit();
    ...
    public static function getOrigin2($id){
        /*echo '
		SELECT * FROM `' . _DB_PREFIX_ . 'inv_packages_historiales` ph '                               
                       . ' join `' . _DB_PREFIX_ . 'inv_origins` o ON ph.id_origin=o.id_origen'
                . ' where ph.id_package_historial=' . $id
		 ;*/
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
		SELECT * FROM `' . _DB_PREFIX_ . 'inv_packages_historiales` ph '                               
...
//----------
    ...
    public static function getPaymentMethod($id){
        //echo "<script>console.log( 'Debug Objects: " . $id . "' );</script>";
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
        SELECT * FROM `pr_orders` WHERE reference="' . $id . '"');
